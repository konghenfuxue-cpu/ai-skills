...
截至 2026-09-05，以下项目已经通过隔离测试或真实文件副本测试：
...
- 300 页合集的可逆合并、拆分和删除后清单重排；
- 实际文件超过 2 GiB 的 ZIP64 输入 CBZ 流式合并、拆分与删除验证；
- 超大图片成员不会一次读入内存，合并和拆分使用 1 MiB 流式复制。
...
`cbz-workflow` 当前状态为“稳定（本地核心流程）”。本地元数据、合并、拆分、删除、ZIP64 大成员和数百页合集均已通过隔离或真实副本测试。JMComic 网络搜索和下载依赖外部站点、网络和授权，仍应在每次使用前单独确认。
